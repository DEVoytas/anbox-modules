...
{
	if (!get_vm_area_ptr)
		get_vm_area_ptr = wrapper_kallsyms_lookup_name("get_vm_area");
...
#endif
{
	if (!zap_page_range_ptr)
		zap_page_range_ptr = wrapper_kallsyms_lookup_name("zap_page_range");
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 11, 0)
...
int map_kernel_range_noflush(unsigned long start, unsigned long size, pgprot_t prot, struct page **pages)
{
	if (!map_kernel_range_noflush_ptr)
		map_kernel_range_noflush_ptr = wrapper_kallsyms_lookup_name("map_kernel_range_noflush");
...
void unmap_kernel_range(unsigned long addr, unsigned long size)
{
	if (!unmap_kernel_range_ptr)
		unmap_kernel_range_ptr = wrapper_kallsyms_lookup_name("unmap_kernel_range");
...
struct files_struct *get_files_struct(struct task_struct *task)
{
	if (!get_files_struct_ptr)
		get_files_struct_ptr = wrapper_kallsyms_lookup_name("get_files_struct");
...
void put_files_struct(struct files_struct *files)
{
	if (!put_files_struct_ptr)
		put_files_struct_ptr = wrapper_kallsyms_lookup_name("put_files_struct");
...
struct sighand_struct *__lock_task_sighand(struct task_struct *tsk, unsigned long *flags)
{
	if (!__lock_task_sighand_ptr)
		__lock_task_sighand_ptr = wrapper_kallsyms_lookup_name("__lock_task_sighand");
...
{
	if (!__alloc_fd_ptr)
		__alloc_fd_ptr = wrapper_kallsyms_lookup_name("__alloc_fd");
...
{
	if (!__fd_install_ptr)
		__fd_install_ptr = wrapper_kallsyms_lookup_name("__fd_install");
...
{
	if (!__close_fd_ptr)
		__close_fd_ptr = wrapper_kallsyms_lookup_name("__close_fd_ptr");
...
{
	if (!can_nice_ptr)
		can_nice_ptr = wrapper_kallsyms_lookup_name("can_nice");
...
int security_binder_set_context_mgr(struct task_struct *mgr)
{
	if (!security_binder_set_context_mgr_ptr)
		security_binder_set_context_mgr_ptr = wrapper_kallsyms_lookup_name("security_binder_set_context_mgr");
...
int security_binder_transaction(struct task_struct *from, struct task_struct *to)
{
	if (!security_binder_transaction_ptr)
		security_binder_transaction_ptr = wrapper_kallsyms_lookup_name("security_binder_transaction");
...
int security_binder_transfer_binder(struct task_struct *from, struct task_struct *to)
{
	if (!security_binder_transfer_binder_ptr)
		security_binder_transfer_binder_ptr = wrapper_kallsyms_lookup_name("security_binder_transfer_binder");
...
int security_binder_transfer_file(struct task_struct *from, struct task_struct *to, struct file *file)
{
	if (!security_binder_transfer_file_ptr)
		security_binder_transfer_file_ptr = wrapper_kallsyms_lookup_name("security_binder_transfer_file");
